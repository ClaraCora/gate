...
- 全局关闭“自动检查”后暂停定时发现、健康检查和自动优化；手动操作和状态查看保持可用。
...
| 活动出口轻量健康检查 | 120 秒 | 小型 HTTPS + 出口 IP |
...
- SOCKS 监听范围；只允许 `127.0.0.1` 或 `0.0.0.0`，后者强制启用统一认证
- 全局自动检查开关；状态持久化，关闭后仍保留手动测试和切换
- 数据保留时间、日志级别和探测流量预算
...
WebUI 会话密钥和管理员密码哈希放在 `/etc/gate/secrets.env`。SOCKS 监听地址和可选统一认证
单独保存在 `/etc/gate/socks-auth.json`；因为 sing-box 和内部出口探测必须使用原始凭据，该密码需要可恢复，
...
- SOCKS 固定端口默认监听 `127.0.0.1`；可显式切换到 `0.0.0.0`，但必须启用统一认证。
- 使用者通过 `ssh -L` 访问。
- 不新增公网防火墙入口。
- SOCKS 若公网开放，必须同时配置强认证、来源 IP 白名单、限速和安全评审。
