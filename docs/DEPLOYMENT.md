...
```sh
curl -fL https://github.com/ClaraCora/gate/releases/download/v0.1.9/gate-install.sh \
  -o /tmp/gate-install.sh
sh /tmp/gate-install.sh --version v0.1.9
```
...
例如版本为 `0.1.9` 时，tag 必须为 `v0.1.9`；版本不一致会使工作流立即失败。
