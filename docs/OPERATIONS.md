...
启用后，向该聊天发送 `/status` 可查看各入口的名称、实际出口 IP 和最近 2 小时健康检查成功率，
格式为“名称 - IP - 成功率 成功次数/总次数”，不显示尚未分配出口 IP 的入口。
切换按钮每行两个，已启用但未分配的入口仍可手动切换。状态消息中的“切换”按钮会先显示确认菜单，
确认后才提交对应入口的自动候选切换；也可以发送 `/switch <入口ID>`，例如 `/switch jp`，同样
...
```sh
sh /tmp/gate-install.sh --version v0.1.11
```
