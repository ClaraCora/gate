...
- SOCKS 可选择仅监听 `127.0.0.1` 或监听 `0.0.0.0`，公网监听强制启用统一认证
...
同一设置窗口可把 SOCKS 监听从默认的 `127.0.0.1` 改为 `0.0.0.0`。选择全部网卡时系统会
强制开启用户名和密码认证，并在 HAProxy 配置校验通过后自动重载。公网开放前还必须在 VPS
防火墙或云安全组中只允许可信来源 IP；不要仅依赖 SOCKS 密码直接对全网开放。

页面顶部的“自动检查”开关可暂停定时节点发现、活动出口健康检查和自动优化，关闭状态会持久化。
手动测试、手动切换和状态查看仍可使用。入口“锁定”只阻止自动换节点，仍会接受定时健康检查；
若全部入口已锁定并希望进一步降低控制面负载，需要同时关闭“自动检查”。

## 安全说明

VPN Gate 是公共志愿者网络，出口稳定性、隐私和可用性均没有商业保证。系统必须净化远端
OpenVPN 配置、隔离每条线路、启用断线阻断，并默认只监听 VPS 回环地址。选择公网 SOCKS
监听时必须配合统一认证和来源 IP 防火墙规则。敏感业务仍应全程
...
同时开启 10 个入口时，OpenVPN 与 sing-box 的常驻进程会接近按入口数线性增长。v0.1.5 已将
运行态检查合并为批量 systemd 查询、为接口结果增加短缓存，并阻止新 SSE 连接回放全部历史事件；
关闭“自动检查”后不会再启动定时探测任务，但已建立的隧道和 SOCKS 进程仍会保持运行。
